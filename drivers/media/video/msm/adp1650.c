...
int ADP1650_torchmode_enable(void)
{
	...
	unsigned char buf;
        pr_err("yanwei%s: entry\n", __func__);
	rc = adp1650_write(adp1650_client, 0x02,0x10);
	rc = adp1650_write(adp1650_client, 0x03,0x00);
	rc = adp1650_read(adp1650_client, 0x03,&buf);
	pr_err("0x03 value is %x\n",buf);
	rc = adp1650_write(adp1650_client, 0x04,0x2C);	
	rc = adp1650_read(adp1650_client, 0x04,&buf);
	pr_err("0x04 value is %x\n",buf);
	rc = adp1650_write(adp1650_client, 0x07,0x20);//24			
	rc = gpio_request(19, "adp1650");	
	rc = gpio_direction_output(19, 1);
	if (rc) {
	pr_err("gpio 19 set direction failed\n");
	}	
#endif
	return rc;
}
...
int ADP1650_flashmode_enable(void)
{
	...
	unsigned char buf;
       pr_err("yanwei%s: entry\n", __func__);	
	 // rc = adp1650_setup(adp1650_client);
	rc = adp1650_write(adp1650_client, 0x02,0x0A);
	rc = adp1650_write(adp1650_client, 0x03,0x00);
	rc = adp1650_read(adp1650_client, 0x03,&buf);
	pr_err("yanwei read 0x03 value is %x\n",buf);
	rc = adp1650_write(adp1650_client, 0x04,0x0F);//8f
#if 0	
...
#else
     rc = gpio_tlmm_config(GPIO_CFG(18, 0,
