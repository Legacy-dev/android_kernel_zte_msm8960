...
#define FW_DEFAULT_NAME	"synaptics_fw.img"
...
static int syna_getfwsize(char * firmware_name)
{
	...
	// check default fw file name on SD card
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", FW_PATH_SDCARD, FW_DEFAULT_NAME);
	pfile = filp_open(filepath, O_RDONLY, 0);
	if(IS_ERR(pfile))
	{
		pr_err("error occured while opening file %s.\n", filepath);

		// get fw file name input from command line
		memset(filepath, 0, sizeof(filepath));
		...
		if(IS_ERR(pfile))
		{
			...
			// check fw file name under etc/firmware path
			memset(filepath, 0, sizeof(filepath));
