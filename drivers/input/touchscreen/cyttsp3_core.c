...
static void _cyttsp_init_tch_map(struct cyttsp *ts)
{
	...
	cyttsp_dbg(ts, CY_DBG_LVL_1,"%s:%02X %02X %02X %02X \n",__func__,
		*(ts->tch_map[0].id),
		*(ts->tch_map[1].id),
		*(ts->tch_map[2].id),
		*(ts->tch_map[3].id));
	...
	struct cyttsp_trk *cur_trk)
{
	...
	int tch = 0;
	u16 *pw;
	//printk("_cyttsp_get_tracks cur_tch:%d\n",cur_tch);
	/* extract xy_data for all currently reported touches */
	for (tch = 0; tch < cur_tch; tch++) {
		if(tch!=4){
            ...
            id = (ts->tch_map[2].tch)->z;
            
            cur_trk[id].tch = CY_TCH;
            
            pw = (u16 *)(&((ts->tch_map[3].tch)->z));//!!!
...
static int _cyttsp_xy_worker(struct cyttsp *ts)
{
	...
	for (id = 0, num_sent = 0; id < CY_NUM_TRK_ID; id++) {
		//printk("huangjinyu cur_tch:%d id:%d cur_trk[id].tch:%d\n",cur_tch,id,cur_trk[id].tch);
		if (cur_trk[id].tch) {
			//printk("huangjinyu id:%d\n",id);
...
static void cyttsp_ts_work_func(struct work_struct *work)
{
	...
	int retval = 0;
	//printk("cyttsp_ts_work_func huangjinyu");
	mutex_lock(&ts->data_lock);
			  ...
			  void *data)
{
	...
	if(ts)
		len += sprintf(page + len, "%-20s 0x%02x%02x_0x%02x%02x\n", "firmware version:", 
		ts->sysinfo_data.app_idh, ts->sysinfo_data.app_idl,
		ts->sysinfo_data.app_verh, ts->sysinfo_data.app_verl);	
	else
...
static int detect_device(struct cyttsp *ts)
{
	int i, buf;
	bool ret = 0;
	int buf1[1];
	...
	for (i=0; i<3; i++ )
	{
		...
		buf=ttsp_read_block_data(ts,CY_REG_BASE,1,buf1);
		if ( buf >= 0 ){
			ret = 1;
			break;
		}
	...
	struct device *dev, int irq, char *name)
{
	...
	if ( ts->platform_data->gpio_init) {
		if(ts->platform_data->gpio_init(1)<0)
		{
			pr_err("%s, gpio init failed! \n", __func__);
			goto error_alloc_failed;
		}

		if (ts->platform_data->power) ts->platform_data->power(1);
		msleep(300);
	}
	if ( !detect_device( ts )){
