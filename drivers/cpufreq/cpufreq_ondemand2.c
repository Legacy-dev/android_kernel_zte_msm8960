...
static
#endif
struct cpufreq_governor cpufreq_gov_ondemand2 = {
       .name                   = "ondemand2",
...
//#define ONDEMAND_LOWER_FREQ 1026000//tcd


#define CONFIG_GPU_CONTROL 1
#ifdef CONFIG_GPU_CONTROL
int gpu_busy_counter = 0;
int gpu_busy_stage = 0;
#endif

#define ONDEMAND_LOWER_FREQ 1026000
#define ONDEMAND_MID_FREQ 1242000

extern u32 update_fps(void);
unsigned int zte_counter = 0;
unsigned int zte_phase = 0;

static void update_freq(unsigned int cur)
{
	if ((cur < ONDEMAND_LOWER_FREQ) && (zte_phase != 0)){
		zte_phase = 0;
		zte_counter = 0;
	} else if (cur > ONDEMAND_LOWER_FREQ && cur < ONDEMAND_MID_FREQ && zte_phase != 1){
		zte_phase = 1;
		zte_counter = 2;
	} else if (cur > ONDEMAND_MID_FREQ && zte_phase != 2){
		zte_phase = 2;
		zte_counter = 24;
	}
}
static void step_down(void)
{
	...
	if (zte_counter > 3)
	    zte_counter-=3;
        if (zte_counter<24)//5=>zte_phase 1
	    zte_phase = 1;
    }
}
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	/* we don't inc freq when fps > 40 */
	if ((max_load_freq > dbs_tuners_ins.up_threshold * policy->cur) && (fb_fps<40) && (fb_fps!=0) && (fb_fps!=1)) {
		/* If switching to max speed, apply sampling_down_factor */
		step_up();

		if (zte_phase == 0) {
			/* idle zte_phase */
			if (policy->cpu == 0 && !cpu_online(1) && (notified == 1)){
                       		kobject_uevent_env(&policy->kobj, KOBJ_MOVE, NULL);
				notified = 0;
			}
			dbs_freq_increase(policy, ONDEMAND_LOWER_FREQ);
		} else if (zte_phase == 1){
			dbs_freq_increase(policy, ONDEMAND_MID_FREQ);
			if (zte_counter == 20){
				if (policy->cpu == 0 && !cpu_online(1) && (notified == 0)){
                        		kobject_uevent_env(&policy->kobj, KOBJ_CHANGE, NULL);
					notified = 1;
				}
			}
		} else {
		...
		if (debug_mask)
		    printk("0)cur=%u, fps=%u, counter=%u, phase=%u\n",policy->cur, fb_fps, zte_counter, zte_phase);
		return;
	}
	if (debug_mask)
		printk("1)ccur=%u, fps=%u, counter=%u, phase=%u\n",policy->cur, fb_fps, zte_counter, zte_phase);
	...
	step_down();
	if ((max_load_freq <= dbs_tuners_ins.up_threshold * policy->cur) || (fb_fps >=45) || (fb_fps==0) || (fb_fps==1)){
	     	if ((zte_phase == 0) && (policy->cur > ONDEMAND_LOWER_FREQ)){
			/* idle zte_phase */
			dbs_freq_increase(policy, ONDEMAND_LOWER_FREQ);
		} else if ((zte_phase == 1) && (policy->cur > ONDEMAND_MID_FREQ)){
			dbs_freq_increase(policy, ONDEMAND_MID_FREQ);
		}
	}
	update_freq(policy->cur);

	if ((policy->cur <= ONDEMAND_LOWER_FREQ) && policy->cpu == 0 && !cpu_online(1) && (notified == 1)){
              	kobject_uevent_env(&policy->kobj, KOBJ_MOVE, NULL);
		notified = 0;
	}
...
static int __init cpufreq_gov_dbs_init2(void)
{
	cputime64_t wall;
	u64 idle_time;
	unsigned int i;
	...
	return cpufreq_register_governor(&cpufreq_gov_ondemand2);
}

static void __exit cpufreq_gov_dbs_exit2(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_ondemand2);
...
fs_initcall(cpufreq_gov_dbs_init2);
#else
module_init(cpufreq_gov_dbs_init2);
#endif
module_exit(cpufreq_gov_dbs_exit2);
