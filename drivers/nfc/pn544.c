...
#define PMIC_GPIO_BASE		NR_GPIO_IRQS
...
static long pn544_dev_ioctl(struct file *filp,unsigned int cmd, unsigned long arg)
{
	...
	case PN544_SET_PWR:
		if (arg == 2) {
			...
			printk("%s power on with firmware,irq2=%d\n", __func__,client->irq);
...
static int nxp_pn544_reset(void)
{
	...
	/********************************************************/
     ...
     //0. set NXP_PN544_IRQ_GPIO to input
       if(pn544_dev->irq_gpio>=PMIC_GPIO_BASE)    //pmic_gpio
       {
     		...
     		rc = pm8xxx_gpio_config(pn544_dev->irq_gpio,	&param);
     		if (rc < 0) {
	...
	else if(pn544_dev->irq_gpio>0)  //msm8960_gpio
	{
     ...
     //1. set NXP_PN544_DOWNLOAD_GPIO to 0
      if(pn544_dev->firm_gpio>=PMIC_GPIO_BASE)    //pmic_gpio
      {
                    struct pm_gpio param = {
     		...
     		rc = pm8xxx_gpio_config(pn544_dev->firm_gpio,	&param);
     		if (rc < 0) {
            			printk(KERN_ERR"%s: Could not configure nfc gpio %d\n",
     ...
     else if(pn544_dev->firm_gpio>0)     //msm8960_gpio
     {
      ...
      if(pn544_dev->clock_gpio>=PMIC_GPIO_BASE)    //pmic_gpio
      {
                    struct pm_gpio param = {
         		.direction	= PM_GPIO_DIR_OUT,
         		.output_buffer	= PM_GPIO_OUT_BUF_CMOS,
     		...
     		rc = pm8xxx_gpio_config(pn544_dev->clock_gpio,	&param);
     		if (rc < 0) {
            			printk(KERN_ERR"%s: Could not configure nfc gpio %d\n",
     		...
     		gpio_set_value_cansleep(pn544_dev->clock_gpio, 0);
     }
     else if(pn544_dev->clock_gpio>0)  //msm8960_gpio
     {
     ...
     if(pn544_dev->dcdc_gpio>=PMIC_GPIO_BASE)    //pmic_gpio
      {
                    struct pm_gpio param = {
         		.direction	= PM_GPIO_DIR_OUT,
         		.output_buffer	= PM_GPIO_OUT_BUF_CMOS,
     		...
     		rc = pm8xxx_gpio_config(pn544_dev->dcdc_gpio,	&param);
     		if (rc < 0) {
            			printk(KERN_ERR"%s: Could not configure nfc gpio %d\n",
     		...
     		gpio_set_value_cansleep(pn544_dev->dcdc_gpio, 1);
     }
     else if( pn544_dev->dcdc_gpio>0)  //msm8960_gpio
     {
       ...
       //4. set NXP_PN544_ENABLE_GPIO to 0_1_0
        if(pn544_dev->ven_gpio>=PMIC_GPIO_BASE)    //pm8921_gpio
        {
                    struct pm_gpio param = {
         		.direction	= PM_GPIO_DIR_OUT,
         		.output_buffer	= PM_GPIO_OUT_BUF_CMOS,
     		...
     		rc = pm8xxx_gpio_config(pn544_dev->ven_gpio,	&param);
     		if (rc < 0) {
            			printk(KERN_ERR"%s: Could not configure nfc gpio %d\n",
	...
	else if(pn544_dev->ven_gpio >0)  //msm8960_gpio
       {
